function Isosurfaces( volume, isovalue )
{
    ...
    for ( var z = 0; z < volume.resolution.z - 1; z++ )
    {
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
		   ...
		   /* 
		    var S0 = parseInt(( id0 -0.1)/0.7*255);
		    var S1 = parseInt(( id1 -0.1)/0.7*255);
		    var S2 = parseInt(( id2 -0.1)/0.7*255);
		    var C0 = new THREE.Color().setHex( cmap[ S0 ] );
		    var C1 = new THREE.Color().setHex( cmap[ S1 ] );
		    var C2 = new THREE.Color().setHex( cmap[ S2 ] );
		    geometry.faces.vertexColors.push( C0 );
		    geometry.faces.vertexColors.push( C1 );
		    geometry.faces.vertexColors.push( C2 );
		   */
                }
            }
            cell_index++;
